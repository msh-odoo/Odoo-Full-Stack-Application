...
class EventServices(models.Model):
    ...
    tag_ids = field.Many2many('event.tag', 'Tags')
    company_id = fields.Many2one('res.company', 'Company', store=True)
    booking_ids = fields.One2many('event.registration' 'Registration')
    ...
    def _check_price(self):
        ...
        for record in self:
            if tools.float_compare(record.price, 0.0, precision_digits=2) < 0:
                raise ValidationError(
